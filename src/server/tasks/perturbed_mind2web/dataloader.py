...
from src.server.tasks.mind2web.data_utils.dom_utils import get_tree_repr, prune_tree
...
def format_input_generation(
    ...
    all_keys = []
    all_values = []
    placeholder_template = "__PLACEHOLDER__{}"
    counter = 0
    for choice in choices:
        choice_str = choice[1]
        if choice_str in tree_repr:
            counter += 1
            key = placeholder_template.format(counter)
            tree_repr = tree_repr.replace(choice_str, key)
            all_keys.append(key)
            all_values.append(choice_str)
...
def format_input_multichoice(
    ...
    if do_perturbation:
        gt_choice = choices[gt] if gt != -1 else ""
        random.shuffle(choices)
    ...
    if gt == -1:
        ...
        if do_perturbation:
            gt = choices.index(gt_choice)
            assert gt != -1, f"Ground truth index is -1! {gt_choice} {choices}"
        gt += 1
        current_action_op = sample["operation"]["op"]
        current_action_value = sample["operation"]["value"]
        ...
        if len(sample["pos_candidates"]) != 0 and (
            ...
            if self.mode == "multichoice":
                seq_context, seq_in, seq_out, _ = format_input_multichoice(
                    sample, candidate_ids, gt, seed=self.seed
                )
            else:
