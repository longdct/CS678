...
from src.server.tasks.knowledgegraph.utils.logic_form_util import (
...
path = "/home/long/code/AgentBench/src/server/tasks/knowledgegraph"
...
def get_relations(
    ...
    out_relations = sorted(list(set(out_relations).intersection(set(relations))))
    if do_perturbation:
        set_seed(seed)
        random.shuffle(out_relations)
...
def get_neighbors(
    ...
    domain = range_info[relation]
    rtn_str = f"Observation: variable ##, which are instances of {domain}"
...
def count(variable: Variable, sparql_executor, do_perturbation=False, seed: int = 42):
    """
...
def get_attributes(
    variable: Variable, sparql_executor, do_perturbation=False, seed: int = 42
):
    program = variable.program
    ...
    out_relations = sorted(list(set(out_relations).intersection(set(attributes))))
    if do_perturbation:
        set_seed(seed)
        random.shuffle(out_relations)
