...
class PerturbedKnowledgeGraph(KnowledgeGraph):
    def __init__(self, data_file, sparql_url=None, round=15, **configs):
        ...
        self.n = configs.pop("n_sampling", 10)
        self.item_seeds = list(range(self.n))
        print(self.item_seeds)
    ...
    async def start_sample(
        ...
        for i in range(self.round):
            print(i, execution_messages)
            if i != 0 and not error_flag:
                responses = []
                for exec_msg in execution_messages:
                    del session.history[-1]
                    session.inject({"role": "user", "content": exec_msg})
                    ...
                    temp_history = session.history[-1]
                    session.history = session.history[:-1]
                print(responses)
                c = Counter(responses)
                message = c.most_common()[0][0]
                session.inject(temp_history)
                execution_messages = []
            else:
                message = await session.action()
                if message.status == AgentOutputStatus.AGENT_CONTEXT_LIMIT:
                        ...
                        for function_name in function_names:
                            try:
                                ...
                                for seed in self.item_seeds:
                                    ...
                                    if execution_message not in execution_messages:
                                        execution_messages.append(execution_message)
